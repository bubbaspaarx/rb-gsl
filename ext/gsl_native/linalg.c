                                            ...
                                            int flag)
{
  ...
  GetNArray(argv[0], na);
  if (na->rank < 2)
    rb_raise(rb_eRuntimeError, "rank >= 2 required");
...
static VALUE rb_gsl_linalg_LU_decomposition(int argc, VALUE *argv, VALUE obj, int flag)
{
  ...
  case 0:
    ...
    if (flag == LINALG_DECOMP_BANG)
      return rb_ary_new3(2, objp, INT2FIX(signum));
    else
      return rb_ary_new3(3, objm, objp, INT2FIX(signum));
    break;
  case 1:
    ...
    if (flag == LINALG_DECOMP_BANG)
      ...
      return rb_ary_new3(2, objm, INT2FIX(signum));
    break;
  default:
...
VALUE rb_gsl_linalg_LU_solve(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (flagp == 1 && flagm == 0)
    rb_raise(rb_eArgError, "permutation must be given");
  if (flagp == 0)
    itmp++;
  bb = argv[itmp];
  ...
  if (flagb == 1)
    gsl_vector_free(b);
  if (flagx == 1)
    return Data_Wrap_Struct(VECTOR_ROW_COL(bb), 0, gsl_vector_free, x);
  else
    return argv[argc - 1];
}
...
static VALUE rb_gsl_linalg_LU_invert(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (argc - 1 == itmp)
    ...
    return Data_Wrap_Struct(cgsl_matrix, 0, gsl_matrix_free, inverse);
}
...
static VALUE rb_gsl_linalg_LU_lndet(int argc, VALUE *argv, VALUE obj)
{
  ...
  case T_OBJECT:
    if (argc < 1)
      rb_raise(rb_eArgError, "wrong number of argument (%d for 1)",
               argc);
#ifdef HAVE_NARRAY_H
    if (NA_IsNArray(argv[0]))
...
static VALUE rb_gsl_linalg_LU_sgndet(int argc, VALUE *argv, VALUE obj)
{
  ...
  {
    if (argc - itmp != 1)
      rb_raise(rb_eArgError, "sign must be given");
    sign = FIX2INT(argv[itmp]);
  }
...
static VALUE rb_gsl_linalg_QR_solve_narray(int argc, VALUE *argv, VALUE obj)
{
  ...
  if (argc != 3)
    rb_raise(rb_eArgError, "Usage: QR.solve(qr, tau, b)");
  GetNArray(argv[0], qr);
  GetNArray(argv[1], tau);
  GetNArray(argv[2], b);
...
static VALUE rb_gsl_linalg_QR_LQ_solve(int argc, VALUE *argv, VALUE obj, int flag)
{
  ...
  size = m->size1;
  if (flagm == 0)
  { /* the matrix given is already decomped */
                                            ...
                                            VALUE pp, VALUE bb, int flag)
{
  ...
  {
  case LINALG_QRPT:
    if (CLASS_OF(qq) != cgsl_matrix_Q)
      rb_raise(rb_eTypeError, "not a Q matrix");
    if (CLASS_OF(rr) != cgsl_matrix_R)
      rb_raise(rb_eTypeError, "not a R matrix");
...
static VALUE rb_gsl_linalg_cholesky_solve(int argc, VALUE *argv, VALUE obj)
{
  ...
  case T_OBJECT:
    if (argc != 2)
      rb_raise(rb_eArgError, "wrong number of argument (%d for 2)",
               argc);
#ifdef HAVE_NARRAY_H
    if (NA_IsNArray(argv[0]))
