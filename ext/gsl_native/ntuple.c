...
VALUE rb_gsl_ntuple_close(VALUE obj)
{
  gsl_ntuple *n = NULL;
  Data_Get_Struct(obj, gsl_ntuple, n);
  gsl_ntuple_close(n);
...
VALUE rb_gsl_ntuple_size(VALUE obj)
{
  gsl_ntuple *n = NULL;
  Data_Get_Struct(obj, gsl_ntuple, n);
...
int rb_gsl_ntuple_select_fn_f(void *data, void *p)
{
  ...
  if (NIL_P(params))
    result = rb_funcall(proc, RBGSL_ID_call, 1, vv);
  else
    result = rb_funcall(proc, RBGSL_ID_call, 2, vv, params);
