...
static VALUE rb_gsl_histogram2d_alloc(int argc, VALUE *argv, VALUE klass)
{
  ...
  case 4:
    if (VECTOR_P(argv[0]) && VECTOR_P(argv[2]))
    {
      ...
      xsize = (size_t)FIX2UINT(argv[1]);
      ysize = (size_t)FIX2UINT(argv[3]);
...
static VALUE rb_gsl_histogram2d_memcpy(VALUE obj, VALUE vhdest, VALUE vhsrc)
{
  ...
  CHECK_HISTOGRAM2D(vhdest);
  CHECK_HISTOGRAM2D(vhsrc);
...
static VALUE rb_gsl_histogram2d_accumulate2(int argc, VALUE *argv, VALUE obj)
{
  ...
  Data_Get_Struct(obj, gsl_histogram2d, h);
  if (x < h->xrange[0])
    x = h->xrange[0] + 4 * GSL_DBL_EPSILON;
  if (x > h->xrange[h->nx])
    x = h->xrange[h->nx] - 4 * GSL_DBL_EPSILON;
  if (y < h->yrange[0])
    y = h->yrange[0] + 4 * GSL_DBL_EPSILON;
  if (y > h->yrange[h->ny])
    y = h->yrange[h->ny] - 4 * GSL_DBL_EPSILON;
...
static VALUE rb_gsl_histogram2d_equal_bins_p(VALUE obj, VALUE hh1, VALUE hh2)
{
  ...
  CHECK_HISTOGRAM2D(hh1);
  CHECK_HISTOGRAM2D(hh2);
                                                           ...
                                                           int flag)
{
  ...
  if (flag == -1)
  {
    ...
    for (i = nx - 2;; i--)
    {
      for (j = ny - 2;; j--)
      {
        hi->bin[i * ny + j] = ((gsl_histogram2d_get(hi, i + 1, j) + gsl_histogram2d_get(hi, i, j + 1)) - gsl_histogram2d_get(hi, i + 1, j + 1)) + gsl_histogram2d_get(h, i, j);
        if (j == 0)
          break;
      }
      if (i == 0)
  ...
  else
  {
    hi->bin[0] = h->bin[0];
    for (j = 1; j < ny; j++)
      hi->bin[j] = gsl_histogram2d_get(hi, 0, j - 1) + gsl_histogram2d_get(h, 0, j);
    for (i = 1; i < nx; i++)
      hi->bin[i * ny] = gsl_histogram2d_get(hi, i - 1, 0) + gsl_histogram2d_get(h, i, 0);
    for (i = 1; i < nx; i++)
    {
      for (j = 1; j < ny; j++)
      {
